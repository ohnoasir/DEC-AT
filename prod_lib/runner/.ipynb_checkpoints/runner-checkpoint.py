...
# build_model might modify the cfg, thus clone
    ...
    if resume_baseline:
        # 从 BaselineTrainer 的检查点加载学生模型权重
        student_model = build_model(cfg)
        checkpointer = DetectionCheckpointer(student_model, cfg.OUTPUT_DIR)
    ...
    return EnsembleTSModel(teacher_model, student_model)        return EnsembleTSModel(model_teacher, model)
